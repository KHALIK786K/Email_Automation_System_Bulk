...
// Format a SQLite "YYYY-MM-DD HH:MM:SS" (stored in local time) into a
// readable label. Replacing the space with "T" makes JS parse it as local
// time instead of UTC, so the hour doesn't shift by the timezone offset.
function fmtDate(v) {
  if (!v) return "—";
...
export default function History() {
  ...
  const clearAll = async () => {
    if (!confirm("Clear all email history? This also resets the 'already contacted' memory, so those HRs will look new again. Continue?")) return;
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500 dark:bg-slate-800">
              <tr>
                <th className="px-4 py-3">Student / HR</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">No records found.</td></tr>
              )}
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-400">{r.scheduled_at ? fmtDate(r.scheduled_at) : "—"}</td>
                  <td className="px-4 py-3 text-xs text-slate-400">{fmtDate(r.sent_at || r.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
